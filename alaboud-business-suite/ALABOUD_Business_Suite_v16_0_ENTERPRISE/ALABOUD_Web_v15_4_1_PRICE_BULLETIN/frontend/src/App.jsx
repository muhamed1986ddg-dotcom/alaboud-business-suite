...
function Dashboard({navigate}){
  ...
  useEffect(()=>{
    Promise.all([
      ...
      setRates(rateRows.slice().sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||""))).slice(0,4));
    });
  },[]);
  ...
  return <div className="premium-dashboard">
    ...
    <section className="premium-grid">
      ...
      <div className="premium-summary panel-dark premium-rate-bulletin">
        <div className="section-heading">
          <h3>نشرة الأسعار</h3>
          <button onClick={()=>navigate("rates")}>عرض الكل</button>
        </div>
        <div className="dashboard-rate-list">
          {rates.length?rates.map(rate=><button className="dashboard-rate-row" key={rate.id||`${rate.baseCurrency}-${rate.quoteCurrency}`} onClick={()=>navigate("rates")}>
            <div className="dashboard-rate-pair">
              <strong>{rate.baseCurrency||"CAD"}/{rate.quoteCurrency||"USD"}</strong>
              <small>{String(rate.createdAt||"").slice(0,10)||"آخر تحديث"}</small>
            </div>
            <div className="dashboard-rate-values">
              <span>شراء <b>{Number(rate.buyRate||0).toFixed(4)}</b></span>
              <span>بيع <b>{Number(rate.sellRate||0).toFixed(4)}</b></span>
            </div>
          </button>):<p className="empty-state">لا توجد أسعار مسجلة.</p>}
        </div>
      </div>
    </section>
...
export default function App(){
  ...
  return <div className={`app ${mobileMenuOpen?"mobile-menu-view":"mobile-page-view"}`}>
    <div className="mobile-page-header no-print">
      ...
      <div className="mobile-brand-center"><img className="mobile-header-logo" src="/alaboud-company-logo.webp" alt="شركة العبود التجارية"/><small>v15.4 Final</small></div>
    ...
    <aside>
      ...
      <div className="sidebar-account-box no-print">
        <div>
          ...
          <small>v15.4 Final Mobile</small>
        </div>
