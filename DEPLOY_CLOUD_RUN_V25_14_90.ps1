...
if ($Version -ne "25.14.90") {
    throw "Expected project version 25.14.90 but found $Version"
}
...
Invoke-ReleaseCheck "v25.14.90 release check" "check:v251490"
...
Write-Host "Verifying Cloud Run revision-level minimum instances are disabled..."

$RevisionMinScale = gcloud run services describe alaboud-business-suite-us `
    --region us-central1 `
    --project alaboud-business-suite `
    --format="value(spec.template.metadata.annotations.'autoscaling.knative.dev/minScale')"

if ($LASTEXITCODE -ne 0) {
    throw "Unable to verify Cloud Run revision-level minScale"
}

if ($RevisionMinScale -and $RevisionMinScale.Trim()) {
    throw "Unsafe Cloud Run configuration detected: revision-level minScale=$RevisionMinScale. Deployment stopped to prevent old revisions from running background jobs."
}

Write-Host "Cloud Run revision-level minScale guard: OK"
