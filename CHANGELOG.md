...
- تغيير «حركة رأس المال حسب العملة» إلى «رأس المال حسب العملة 💰».
- إضافة المضاف، المسحوب، رأس المال الحالي، الخارج للتشغيل، العائد من التشغيل، والعالق لكل عملة.
- إضافة معدل دوران رأس المال لكل عملة اعتمادًا على أصل الأموال التي خرجت في الحوالات ثم عاد منها ما تم تحصيله فعليًا.
- استبعاد الأرباح والرسوم وإضافات/سحوبات المالك من حساب الدوران، ودعم الدفعات الجزئية بدون احتساب مزدوج.
- حساب عودة رأس المال للحوالات ضمن الشهر المحدد حتى نهاية ذلك الشهر حتى تبقى التقارير التاريخية ثابتة.

## v25.14.84 — Unified Company Debt Calculation

- توحيد معادلة أرصدة الشركات بين صفحة الشركات والدين العام والميزانية والجرد عبر `CompanyDebtPosition`.
- تصفية حركات «لنا/علينا» المحلية أولًا لكل شركة ولكل عملة، ثم تصنيف الرصيد النهائي إلى دين لنا أو دين علينا.
- توحيد التحويل إلى CAD على مخطط أسعار الصرف الآلية نفسه؛ لم يعد الدين العام يستخدم مخططًا ثانيًا يشمل الأسعار اليدوية.
- إبقاء الأرصدة الخارجية متعددة العملات كما تصل من الموصلات، مع منع تكرار الحركات المرتبطة مباشرة برصيد خارجي.
- إضافة اختبار يعيد تمثيل فرق 857.22 CAD ويتحقق من تطابق الشركات والدين العام والميزانية والجرد حسابيًا.
- لم تتغير موصلات أو مزامنة Jad أو Tawasul أو Dahab أو Suryana، ولا يوجد migration أو تعديل تلقائي للبيانات التاريخية.

## v25.14.83 — Rate Limit & Exchange Refresh Reliability

- اعتراض Android لاستجابة `429 Rate exceeded` بدل الشاشة البيضاء مع محاولتي إعادة اتصال فقط.
- جلب مصدري الأسعار بالتوازي وحفظ الأسعار الناجحة في عملية قاعدة بيانات واحدة.
- إيقاف التحديث الساعي المتكرر من كل نافذة متصفح والاحتفاظ بآخر أسعار صحيحة عند الفشل.
- لم تتغير موصلات أو مزامنة Jad أو Tawasul أو Dahab أو Suryana.

## v25.14.82 — Live Budget Refresh

- إصلاح زر «تحديث» في صفحة الميزانية ليعرض حالة التنفيذ ويؤكد اكتمال القراءة.
- فرض قراءة جديدة لمؤشرات الميزانية وحركات رأس المال والأسعار والجرد بدل احتمال استخدام استجابة قديمة من الواجهة أو الوسيط.
- إبقاء آخر ميزانية صحيحة ظاهرة إذا فشل طلب تحديث عابر بدل إفراغ الصفحة.
- منع استجابة أقدم من الكتابة فوق نتيجة تحديث أحدث عند تزامن أكثر من طلب.
- تنظيف ذاكرة GET في الواجهة بعد اكتمال مهمة مزامنة رصيد شركة مرتبطة بنجاح.
- إضافة اختبار رقمي يثبت انتقال إجمالي أرصدة العملاء والشركات من 300 إلى 450 بعد تعديل القيم.
- لم تتغير معادلات الجرد أو صافي رأس المال، ولم تتغير موصلات Jad أو Tawasul أو Dahab أو Suryana أو طريقة عملها.
- لا يوجد migration لقاعدة البيانات ولا تعديل آلي للبيانات التاريخية.

## v25.14.81 — Comprehensive Net Capital Correction

- إعادة صافي ديون العملاء والشركات إلى الرقم الرئيسي «صافي رأس المال الشامل» كما كان في السلوك المعتمد سابقًا.
- اعتماد المعادلة: رأس المال المضاف − المسحوبات + الأرباح − المصروفات − توزيعات الأرباح + الدين لنا − الدين علينا.
- إبقاء صافي حقوق الملكية منفصلًا داخليًا لاستخدام المطابقة الرقابية في الجرد، من دون إسقاط الذمم من واجهة رأس المال.
- استخدام إجماليات الذمم الشاملة المحمية من التكرار نفسها المستخدمة في الجرد الشهري.
- إضافة اختبار رقمي مطابق للقيم الظاهرة في النسخة السابقة، ونتيجته 177,176.17 CAD.
- لم يُنشر هذا الإصدار ولم تُحوّل إليه حركة إنتاجية.

## v25.14.80 — Exact Accounting Inventory

- فصل صفحة الميزانية إلى أرقام تراكمية، نتيجة شهرية، ذمم شاملة، وجرد نهائي مثبت.
- إصلاح فرق السنت عبر تركيب الإجماليات من بنود مقربة إلى سنت CAD بطريقة حتمية.
- منع حساب جرد نهائي بافتراض أن كاش الخزنة صفر.
- إزالة المؤشرات التقديرية غير الدفترية من سطح الميزانية المحاسبي.
- إضافة اختبارات رقمية مستقلة للجرد والذمم وصافي رأس المال.
- لم يُنشر هذا الإصدار ولم تُحوّل إليه حركة إنتاجية.

## v25.14.79 — Capital Equity Consistency

- تصحيح صفحة رأس المال التي كانت تشرح صافي رأس المال بمعادلة قديمة تخلط الذمم والمصروفات مع حقوق الملكية.
- اعتماد المعادلة المرئية نفسها المستخدمة في المحرك: المال المضاف − المسحوبات + صافي الربح المحقق − توزيعات الأرباح.
- فصل الذمم لنا والذمم علينا في بطاقة مستقلة باسم «صافي الذمم» وعدم عرضها كجزء من صافي رأس المال.
- توحيد إجماليات الذمم في صفحة رأس المال مع منطق الجرد الشامل ومنع التكرار المباشر، بما في ذلك الذمم اليدوية.
- رفض v25.14.78 قبل تحويل حركة الإنتاج؛ لم يخدم ذلك الإصدار أي مستخدم إنتاج.

## v25.14.78 — Financial Inventory Integrity

- تصحيح قيمة الجرد لتساوي صافي الأصول الفعلية: الخزنة والأرصدة والذمم لنا ناقص الذمم علينا، من دون إضافة رأس المال إلى الخزنة مرة ثانية.
- فصل صافي رأس المال الدفتري عن قيمة الجرد، وإظهار فرق الجرد كمؤشر رقابي مستقل بلا تسوية آلية.
- إدراج الذمم اليدوية لنا وعلينا بصورة متناظرة، واستبعاد التكرار فقط عند وجود معرف أو مرجع مصدر مباشر، مع تعليم حالات تشابه الأسماء للمراجعة دون حذف تاريخي.
- منع جمع إيصال العميل مرة ثانية مع توزيعاته الفرعية في التقرير الشهري.
- دعم أجرة الحوالة بطريقتين معتمدتين: ربح فرق السعر، أو أجرة CAD مدفوعة مستقلة فوق مبلغ الحوالة؛ وإلغاء خصم أجرة CAD مباشرة من مبلغ أجنبي.
- إضافة اختبارات سلوكية رقمية مستقلة للجرد ورأس المال والدفعات والذمم وأرباح الحوالات، وتصحيح مسار اختبار Windows.
- لا يوجد migration لقاعدة البيانات، ولا حذف أو تعديل آلي للبيانات التاريخية.

## v25.14.77 — Mobile Customer Statement Cards

- استبدال الجدول العريض داخل كشف حساب العميل ببطاقات هاتف مخصصة تمنع الانزلاق الأفقي وقص القيم خارج الشاشة.
- عرض كل حوالة ودفعة بعناوين واضحة وحقول ثابتة كاملة العرض، مع التفاف آمن للنصوص والأرقام الطويلة.
- إبقاء جدول سطح المكتب والطباعة كما هو، وإخفاء بطاقات الهاتف عند الطباعة.
- منع أي عنصر داخل كشف الحساب من توسيع الصفحة خارج عرض الهاتف.
- إضافة اختبارات انحدار مستقلة لتخطيط الهاتف ومنع overflow. لا تغيير في البيانات أو المعادلات المالية.

## v25.14.76 — Customer Statement Contrast

- إصلاح تعارض الألوان الذي جعل مبالغ ونصوص كشف حساب العميل بيضاء فوق خلفية بيضاء على الهاتف.
- تحويل شاشة كشف الحساب إلى سطح داكن عالي التباين ومتناسق مع تصميم التطبيق، مع عناوين ذهبية ونصوص فاتحة ومبالغ الدفعات خضراء والإجمالي النهائي ذهبي.
- إضافة تنسيق محكم للجداول والحساب القديم والدفعات والإجمالي النهائي لمنع قواعد الألوان العامة من إخفاء القيم.
- إبقاء الطباعة وحفظ PDF بخلفية بيضاء ونص أسود واضح لتوفير الحبر.
- إضافة اختبار انحدار منفصل لألوان الشاشة والطباعة. لا تغيير في البيانات أو الحسابات المالية.

## v25.14.75 — Mobile Edit & Centered Dialogs

- إصلاح أزرار التعديل في صفحات الحوالات والعملاء والدفعات والمصروفات والشركات؛ نموذج التعديل أصبح يُكشف داخل حاوية تمرير التطبيق الفعلية على الهاتف بدل محاولة تمرير نافذة المتصفح غير القابلة للتمرير.
- إصلاح تعديل الشركة من الصفحة الموحدة بحيث يفتح نموذج الشركة المخفي قبل نقله إلى موضع العرض.
- نقل جميع نوافذ التأكيد المشتركة، ومنها حذف السجلات وتصفير حساب العميل، من أسفل الهاتف إلى منتصف الشاشة.
- إبقاء محتوى النافذة قابلًا للتمرير وتثبيت أزرار الإجراءات داخل المساحة الآمنة بعيدًا عن شريط تنقل Android.
- استبدال تأكيدات المتصفح القديمة في حذف الدين وإغلاق الجرد بنافذة التأكيد الموحدة.
- إضافة اختبارات انحدار لمسارات التعديل وتموضع النوافذ على الهاتف. لا تغيير في الحسابات المالية أو مخطط قاعدة البيانات.
