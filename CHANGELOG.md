...
- Treat ENOTFOUND, EAI_AGAIN and related DNS/network failures as transient PostgreSQL errors.
- Retry the full PostgreSQL startup and migration sequence.
- Start the HTTP listener immediately so Render does not fail the deployment while PostgreSQL recovers.
- Return a retryable 503 for business API calls until durable storage is ready.
- Keep /api/health reachable during recovery and expose the real readiness state in JSON.
- Disable ephemeral JSON fallback in production when DATABASE_URL is configured.
