const fs = require('fs');
const path = require('path');
const assert = require('assert');
const source = fs.readFileSync(path.join(__dirname,'adapters','PostgresStateAdapter.js'),'utf8');
assert(source.includes("WITH saved AS"), 'durable save should combine revision guard and write in one SQL statement');
assert(source.includes("receipt AS ("), 'idempotency receipt should be part of the combined SQL statement');
assert(!source.includes("SELECT revision FROM app_state WHERE state_key='main' FOR UPDATE"), 'redundant pre-write row-lock round trip should be removed');
assert(source.includes('PG_INTERACTIVE_WRITE_RETRIES || 2'), 'interactive retry count should be bounded');
assert(source.includes('PG_INTERACTIVE_WRITE_BUDGET_MS || 7000'), 'interactive retry budget should be bounded');
assert(source.includes('PG_INTERACTIVE_HARD_STEP_TIMEOUT_MS || 30000'), 'each interactive database step must be bounded');
const api = fs.readFileSync(path.join(__dirname,'..','..','..','frontend','src','api.js'),'utf8');
assert(api.includes('method==="get"?45000:12000'), 'frontend should begin ambiguous commit recovery promptly');
console.log('Low-latency durable operations policy tests passed');
