...
const { rebuildTreasury, upsertTransferMovement, cancelTransferMovement, treasuryProfitForRange } = require("./Treasury");
...
upsertTransferMovement(transferStore, tx, {...helpers, deliveryRate:2.5});
...
assert.equal(treasuryProfitForRange(transferStore,{from:"2026-01-01",to:"2026-01-31"}), 20);
...
assert.equal(originalTransferProfit+treasuryProfitForRange(transferStore,{from:"2026-01-01",to:"2026-01-31"}),28,"net profit includes treasury FX exactly once");
...
assert.equal(transitionStore.treasuryMovements.filter(row=>row.sourceKey==="TRANSFER:changing").length,1,"IN to OUT reuses the linked movement");
assert.equal(rebuildTreasury(transitionStore)[0].balance,80);
cancelTransferMovement(transitionStore,"changing",helpers);
assert.equal(rebuildTreasury(transitionStore)[0].balance,100,"NONE cancels the linked effect");
changing.treasuryEffect="IN";
linked=upsertTransferMovement(transitionStore,changing,helpers);
assert.equal(linked.isCancelled,false,"NONE to IN reactivates the same movement");
