...
(async()=>{
  ...
  try{
    ...
    r=await request.post(`${baseUrl}/api/transactions`,{data:{customerId:customer.id,currency:"USD",amount:100,costRate:1.3,finalRate:1.4,transferFee:10,feeMethod:"PAID",transferDate:new Date().toISOString().slice(0,10)}});
    assert.equal(r.status(),201); const tx=await r.json();
    r=await request.patch(`${baseUrl}/api/transactions/${tx.id}`,{data:{finalRate:1.45}});assert.equal(r.status(),200);
