...
function request(method,route,body,token){return new Promise((resolve,reject)=>{const data=body?JSON.stringify(body):"";const req=http.request({hostname:"127.0.0.1",port,path:route,method,headers:{"Content-Type":"application/json",...(token?{Authorization:`Bearer ${token}`}:{})}},res=>{let out="";res.on("data",c=>out+=c);res.on("end",()=>{let parsed={};try{parsed=out?JSON.parse(out):{}}catch{parsed={raw:out}}resolve({status:res.statusCode,body:parsed})})});req.on("error",reject);if(data)req.write(data);req.end()})}
