...
app.post("/api/transactions", auth, (req,res)=>{
  const {customerId,direction="CAD_TO_USD",amount,costRate,finalRate,transferFee=0,feeMethod="ADD",status="COMPLETED",transferDate=""}=req.body||{};
  const nums=[amount,costRate,finalRate,transferFee].map(Number); if(nums.some(n=>!Number.isFinite(n))||nums[0]<=0||nums[1]<=0||nums[2]<=0||nums[3]<0)return res.status(400).json({message:"Invalid transaction values"});
  const [a,cost,final,fee]=nums; const destination=direction==="CAD_TO_USD"?a*final:a/final; const exchangeProfit=direction==="CAD_TO_USD"?a*(cost-final):a*(final-cost); const totalProfit=exchangeProfit+fee; const totalCustomerDue=feeMethod==="ADD"?a+fee:a; const beneficiaryReceives=feeMethod==="DEDUCT"?Math.max(destination-fee,0):destination;
  const tx=mutate((s)=>{if(!s.customers.some(c=>c.id===customerId))throw new Error("Customer not found");const n=s.transactions.length+1;const t={id:id(),number:`TRX-${new Date().getFullYear()}-${String(n).padStart(6,"0")}`,customerId,direction,amount:+a.toFixed(2),costRate:cost,finalRate:final,transferFee:+fee.toFixed(2),feeMethod,destinationAmount:+destination.toFixed(2),beneficiaryReceives:+beneficiaryReceives.toFixed(2),exchangeProfit:+exchangeProfit.toFixed(2),totalProfit:+totalProfit.toFixed(2),totalCustomerDue:+totalCustomerDue.toFixed(2),status,transferDate:transferDate||new Date().toISOString().slice(0,10),createdAt:now(),createdBy:req.user.id};s.transactions.push(t);audit(s,req.user.id,"CREATE","TRANSACTION",t.id);return t;});
...
app.patch("/api/transactions/:id", auth, (req,res)=>{
  try{
    const updated=mutate((s)=>{
      ...
      const allowed=["amount","costRate","finalRate","transferFee","feeMethod","transferDate","direction","status"];
      ...
      const destination=transaction.direction==="CAD_TO_USD"?amount*finalRate:amount/finalRate;
      const exchangeProfit=transaction.direction==="CAD_TO_USD"
        ? amount*(cost-finalRate)
        : amount*(finalRate-cost);

      transaction.destinationAmount=+destination.toFixed(2);
      transaction.beneficiaryReceives=+(transaction.feeMethod==="DEDUCT"?Math.max(destination-fee,0):destination).toFixed(2);
      transaction.exchangeProfit=+exchangeProfit.toFixed(2);
      transaction.totalProfit=+(exchangeProfit+fee).toFixed(2);
      transaction.totalCustomerDue=+(transaction.feeMethod==="ADD"?amount+fee:amount).toFixed(2);
