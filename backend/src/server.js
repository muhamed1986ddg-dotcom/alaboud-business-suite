...
function seedAdmin() {
  mutate((store) => {
    let company = store.companies.find(
      (item) => item.slug === "alaboud-primary",
    );
    if (!company) {
      company = {
        id: id(),
        name: "شركة العبود التجارية",
        slug: "alaboud-primary",
        phone: "",
        active: true,
        createdAt: now(),
      };
    ...
    let admin = store.users.find(
      (user) => user.email === "admin@alaboud.local",
    );
    if (!admin) {
      admin = {
        id: id(),
    ...
    if (!store.companySettings[company.id]) {
      store.companySettings[company.id] = {
        ...(store.notificationSettings || {}),
        overdueDays: store.notificationSettings?.overdueDays || 7,
        lowCashLimit: store.notificationSettings?.lowCashLimit || 5000,
        whatsappTemplate: store.notificationSettings?.whatsappTemplate || "",
      };
    }
  });
}
...
function auth(req, res, next) {
  ...
  try {
    ...
    if (!req.user.companyId)
      return res.status(401).json({ message: "Company account required" });
...
function customerSummary(store, c) {
  const overdueThreshold = Math.max(
    1,
    safeNumber(store.notificationSettings?.overdueDays, 7),
  );
  const transactions = (
    Array.isArray(store.transactions) ? store.transactions : []
  ).filter((item) => item && !item.isDeleted);
  const payments = (Array.isArray(store.payments) ? store.payments : []).filter(
    (item) => item && !item.isDeleted,
  );

  const txs = transactions.filter(
    (transaction) =>
      transaction.customerId === c.id && transaction.status !== "CANCELLED",
  );
  ...
  for (const payment of payments) {
    paymentByTransaction.set(
      ...
      safeNumber(paymentByTransaction.get(payment.transactionId)) +
        safeNumber(payment.amount),
    );
  }

  const oldBalance = Math.max(safeNumber(c.oldBalance), 0);
  const oldBalancePaid = Math.min(
    Math.max(safeNumber(c.oldBalancePaid), 0),
    oldBalance,
  );
  ...
  for (const transaction of txs) {
    ...
    const transactionPaid = safeNumber(
      paymentByTransaction.get(transaction.id),
    );
    ...
    if (remaining > 0) {
      ...
      if (!Number.isNaN(transactionDate.getTime())) {
        ...
        if (!oldestUnpaidDate || dateText < oldestUnpaidDate)
          oldestUnpaidDate = dateText;
      }
    }
  }
...
app.post("/api/auth/login", (req, res) => {
  ...
  const user = store.users.find(
    (u) =>
      u.email.toLowerCase() === String(email || "").toLowerCase() && u.active,
  );
  if (!user || !bcrypt.compareSync(String(password || ""), user.passwordHash)) {
    return res.status(401).json({ message: "Invalid credentials" });
  }
  const company = store.companies.find(
    (item) => item.id === user.companyId && item.active,
  );
  if (!company)
...
app.post("/api/auth/register-company", (req, res) => {
  ...
  if (!ownerName || !companyName || !email.includes("@")) {
    return res
      .status(400)
      .json({ message: "الاسم واسم الشركة والبريد الإلكتروني مطلوبة" });
  }
  ...
  try {
    ...
    res
      .status(400)
      .json({ message: error.message || "تعذر إنشاء حساب الشركة" });
  }
});
...
app.post("/api/auth/change-password", auth, (req, res) => {
  ...
  try {
    mutate((store) => {
      const user = store.users.find(
        (item) => item.id === req.user.id && item.active,
      );
    ...
    res
      .status(400)
      .json({ message: error.message || "تعذر تغيير كلمة المرور" });
  }
});
...
app.patch("/api/company-profile", auth, (req, res) => {
  if (req.user.role !== "ADMIN")
    return res
      .status(403)
      .json({ message: "تعديل هوية الشركة متاح للمسؤول الكامل فقط" });
  ...
  if (
    logoDataUrl &&
    !/^data:image\/(png|jpeg|jpg|webp);base64,/i.test(logoDataUrl)
  ) {
    return res.status(400).json({ message: "صيغة الشعار غير مدعومة" });
  }
  if (logoDataUrl.length > 1500000)
    return res.status(400).json({ message: "حجم الشعار كبير جدًا" });
  const company = mutate((store) => {
    const item = store.companies.find(
      (company) => company.id === req.user.companyId,
    );
...
app.post("/api/users", auth, (req, res) => {
  ...
  const email = String(req.body?.email || "")
    ...
    .toLowerCase();
  const password = String(req.body?.password || "");
  const role = ["ADMIN", "MANAGER", "USER"].includes(
  ...
  if (!name || !email || !email.includes("@")) {
    return res
      .status(400)
      .json({ message: "الاسم والبريد الإلكتروني مطلوبان" });
  }
  if (password.length < 8) {
    return res
      .status(400)
  ...
  try {
    const created = mutate((store) => {
      ...
      store.users.push(user);
      audit(store, req.user.id, "CREATE", "USER", user.id, {
        name,
        email,
        role,
      });
...
app.get("/api/dashboard", auth, (_req, res) => {
  ...
  const todayTx = s.transactions.filter(
    (t) => t.createdAt.slice(0, 10) === today && t.status !== "CANCELLED",
  );
  const todayExpenses = s.expenses
    .filter((e) => e.date === today)
    .reduce((a, e) => a + Number(e.amount), 0);
  const totalProfit =
    todayTx.reduce((a, t) => a + Number(t.totalProfit || 0), 0) - todayExpenses;
  const receivables = s.customers.reduce(
    (a, c) => a + customerSummary(s, c).finalBalance,
    0,
  );
  const capital = s.capitalMovements.reduce(
    (a, m) => a + (m.type === "IN" ? Number(m.amount) : -Number(m.amount)),
    0,
  );
  res.json({
    ...
    recent: todayTx.slice(-8).reverse(),
  });
});
...
app.get("/api/notification-settings", auth, (_req, res) => {
  ...
  res.json({
    ...
    lowCashLimit: Math.max(
      0,
      safeNumber(store.notificationSettings?.lowCashLimit, 5000),
    ),
    whatsappTemplate: String(
      store.notificationSettings?.whatsappTemplate || "",
    ),
  });
});
  ...
  const updated = mutate((store) => {
    ...
    if (req.body?.overdueDays !== undefined) {
      ...
      if (!Number.isFinite(value) || value < 1 || value > 365)
        throw new Error("مدة التأخير يجب أن تكون بين 1 و365 يومًا");
    ...
    if (req.body?.lowCashLimit !== undefined) {
      ...
      if (!Number.isFinite(value) || value < 0)
        throw new Error("حد السيولة غير صحيح");
      store.notificationSettings.lowCashLimit = value;
    }
    if (req.body?.whatsappTemplate !== undefined) {
      store.notificationSettings.whatsappTemplate = String(
...
app.get("/api/notifications", auth, (_req, res) => {
  const store = readStore();
  const customers = (Array.isArray(store.customers) ? store.customers : []).map(
  ...
  const capital = (
    Array.isArray(store.capitalMovements) ? store.capitalMovements : []
  ).reduce(
    (sum, item) =>
      sum +
      (item.type === "IN" ? safeNumber(item.amount) : -safeNumber(item.amount)),
    0,
  );
  const lowCashLimit = Math.max(
    0,
    safeNumber(store.notificationSettings?.lowCashLimit, 5000),
  );
  ...
  res.json({
    ...
    overdueTotal: +overdue
      .reduce((sum, item) => sum + safeNumber(item.finalBalance), 0)
      .toFixed(2),
    notifications,
  });
});
    ...
    const item = {
      ...
      expectedAmount:
        expectedAmount === null || expectedAmount === ""
          ? null
          : +safeNumber(expectedAmount).toFixed(2),
      createdAt: now(),
...
app.get("/api/notification-actions/:customerId", auth, (req, res) => {
  const store = readStore();
  const rows = (
    Array.isArray(store.notificationActions) ? store.notificationActions : []
  )
...
app.get("/api/customer-alerts", auth, (_req, res) => {
  ...
  const actions = Array.isArray(store.notificationActions)
    ? store.notificationActions
    : [];
  const today = new Date().toISOString().slice(0, 10);
  ...
  const rows = (Array.isArray(store.customers) ? store.customers : [])
    .map((customer) => {
      ...
      const customerPayments = payments
        ...
        .sort((a, b) =>
          String(b.paymentDate || b.createdAt).localeCompare(
      ...
      const customerTransactions = (
        ...
        .map((transaction) => {
          const paid = payments
            .filter(
              (payment) =>
                payment?.transactionId === transaction.id && !payment.isDeleted,
            )
            .reduce((sum, payment) => sum + safeNumber(payment.amount), 0);
          return {
            transaction,
            remaining: Math.max(
              safeNumber(transaction.totalCustomerDue) - paid,
              0,
            ),
          };
        })
        ...
        .sort((x, y) =>
          String(
            y.transaction.transferDate || y.transaction.createdAt || "",
          ).localeCompare(
            String(x.transaction.transferDate || x.transaction.createdAt || ""),
          ),
        );
      ...
      return {
        ...
        lastPaymentDate: customerPayments[0]
          ? String(
              customerPayments[0].paymentDate || customerPayments[0].createdAt,
            ).slice(0, 10)
          : null,
        lastTransferNumber: latestUnpaid?.transaction?.number || null,
        lastTransferDate: latestUnpaid
          ? String(
              latestUnpaid.transaction.transferDate ||
                latestUnpaid.transaction.createdAt ||
                "",
            ).slice(0, 10)
          : null,
        lastTransferAmount: latestUnpaid
          ? +safeNumber(latestUnpaid.transaction.totalCustomerDue).toFixed(2)
          : 0,
        lastTransferRemaining: latestUnpaid
          ? +safeNumber(latestUnpaid.remaining).toFixed(2)
          : 0,
        latestAction,
        promiseDate: latestAction?.promiseDate || null,
        expectedAmount: latestAction?.expectedAmount ?? null,
        contacted:
          latestAction?.action === "CONTACTED" ||
          latestAction?.action === "PROMISE_TO_PAY",
      };
    })
    .filter((customer) => customer.overdue)
    .sort((a, b) => b.overdueDays - a.overdueDays);
  ...
  const largestBalance = rows.reduce(
    (max, item) =>
      safeNumber(item.finalBalance) > safeNumber(max?.finalBalance)
        ? item
        : max,
    null,
  );
  ...
  res.json({
    ...
    totalOverdue: +rows
      .reduce((sum, item) => sum + safeNumber(item.finalBalance), 0)
      .toFixed(2),
    largestOverdueBalance: largestBalance
      ? +safeNumber(largestBalance.finalBalance).toFixed(2)
      : 0,
...
app.get("/api/capital-overview", auth, (req, res) => {
  ...
  const capitalMovements = Array.isArray(store.capitalMovements)
    ? store.capitalMovements
    : [];
  ...
  const debtPayments = Array.isArray(store.generalDebtPayments)
    ? store.generalDebtPayments
    : [];
  ...
  const monthTransactions = transactions.filter(
    (item) =>
      String(item.transferDate || item.createdAt || "").slice(0, 7) ===
      requestedMonth,
  );
  ...
  const monthlyProfit = monthTransactions.reduce(
    (sum, item) => sum + safeNumber(item.totalProfit),
    0,
  );
  const monthlyExpenses = expenses
    .filter(
      (item) =>
        String(item.date || item.createdAt || "").slice(0, 7) ===
        requestedMonth,
    )
  ...
  const receivables = customers.reduce(
    (sum, customer) =>
      sum + safeNumber(customerSummary(store, customer).finalBalance),
    0,
  );
  ...
  for (const payment of debtPayments) {
    debtPaidById.set(
      payment.debtId,
      safeNumber(debtPaidById.get(payment.debtId)) + safeNumber(payment.amount),
    );
  }
  ...
  for (const debt of debts) {
    const remaining = Math.max(
      safeNumber(debt.amount) - safeNumber(debtPaidById.get(debt.id)),
      0,
    );
  ...
  const totalCapital =
    ...
    generalPayable;
  const turnoverBase =
    Math.abs(capitalBalance) > 0
      ? Math.abs(capitalBalance)
      : Math.abs(totalCapital);
  const turnoverRate =
    turnoverBase > 0 ? monthlyTransferValue / turnoverBase : 0;
  const averageTransfer = monthTransactions.length
    ? monthlyTransferValue / monthTransactions.length
    : 0;
...
app.get("/api/monthly-report", auth, (req, res) => {
  ...
  const transactions = (
    ...
    .filter(
      (item) =>
        String(item.transferDate || item.createdAt || "").slice(0, 7) === month,
    );

  const expenses = (Array.isArray(store.expenses) ? store.expenses : []).filter(
    (item) => String(item.date || item.createdAt || "").slice(0, 7) === month,
  );
  ...
  const payments = (Array.isArray(store.payments) ? store.payments : [])
    ...
    .filter(
      (item) =>
        String(item.paymentDate || item.date || item.createdAt || "").slice(
          0,
          7,
        ) === month,
    );
  ...
  const feesTotal = transactions.reduce(
    (sum, item) => sum + safeNumber(item.transferFee),
    0,
  );
  const exchangeProfit = transactions.reduce(
    (sum, item) => sum + safeNumber(item.exchangeProfit),
    0,
  );
  ...
  for (const transaction of transactions) {
    customerMap.set(
      ...
      safeNumber(customerMap.get(transaction.customerId)) +
        safeNumber(transaction.amount),
    );
  }
  ...
  const topCustomers = Array.from(customerMap.entries())
    .map(([customerId, total]) => ({
      customerId,
      customerName:
        (Array.isArray(store.customers) ? store.customers : []).find(
          (c) => c.id === customerId,
        )?.name || "-",
  ...
  res.json({
    ...
    summary: {
      ...
      averageTransfer: +(
        transactions.length ? transferTotal / transactions.length : 0
      ).toFixed(2),
      largestTransfer: +(
        transactions.length
          ? Math.max(...transactions.map((item) => safeNumber(item.amount)))
          : 0
      ).toFixed(2),
      smallestTransfer: +(
        transactions.length
          ? Math.min(...transactions.map((item) => safeNumber(item.amount)))
          : 0
      ).toFixed(2),
    ...
    transactions: transactions
      .slice()
      .sort((a, b) =>
        String(a.transferDate || a.createdAt).localeCompare(
          String(b.transferDate || b.createdAt),
        ),
      ),
  });
});
...
app.post("/api/customers", auth, (req, res) => {
  ...
  const customer = mutate((s) => {
    const c = {
      ...
      identityNumber,
      notes,
      oldBalance: +Math.max(safeNumber(oldBalance), 0).toFixed(2),
      oldBalancePaid: 0,
      createdAt: now(),
    };
    s.customers.push(c);
    audit(s, req.user.id, "CREATE", "CUSTOMER", c.id);
    return c;
  });
...
app.patch("/api/customers/:id", auth, (req, res) => {
  try {
    const updated = mutate((store) => {
      const customer = (
        Array.isArray(store.customers) ? store.customers : []
      ...
      customer.oldBalance = +Math.max(
        safeNumber(customer.oldBalance),
        0,
      ).toFixed(2);
      customer.oldBalancePaid = +Math.min(
        Math.max(safeNumber(customer.oldBalancePaid), 0),
        customer.oldBalance,
      ).toFixed(2);
      customer.updatedAt = now();
      customer.updatedBy = req.user.id;
      audit(store, req.user.id, "UPDATE", "CUSTOMER", customer.id, {
        oldData,
        newData: { ...customer },
      });
      return customer;
...
app.get("/api/customers/:id", auth, (req, res) => {
  try {
    ...
    const allPayments = (
      Array.isArray(store.payments) ? store.payments : []
    ).filter((item) => !item?.isDeleted);

    const customer = customers.find(
      (item) => item && item.id === req.params.id,
    );
    if (!customer) return res.status(404).json({ message: "العميل غير موجود" });

    const transactions = allTransactions
      .filter(
        (transaction) => transaction && transaction.customerId === customer.id,
      )
      .map((transaction) => {
    ...
    s.transactions
      ...
      .map((t) => {
        const paidAmount = s.payments
          .filter(
            (payment) => payment.transactionId === t.id && !payment.isDeleted,
          )
          .reduce((sum, payment) => sum + safeNumber(payment.amount), 0);
        const remaining = Math.max(
          safeNumber(t.totalCustomerDue) - paidAmount,
          0,
        );
        return {
          ...t,
          customerName:
            s.customers.find((c) => c.id === t.customerId)?.name || "-",
...
app.post("/api/transactions", auth, (req, res) => {
  ...
  const totalCustomerDue =
    feeMethod === "ADD" ? baseCustomerDue + fee : baseCustomerDue;
  ...
  const tx = mutate((s) => {
    if (!s.customers.some((c) => c.id === customerId))
  ...
  try {
    ...
    const result = mutate((store) => {
      ...
      const rows = store.transactions
        ...
        .map((transaction) => {
          ...
          return {
            transaction,
            remaining: Math.max(
              Number(transaction.totalCustomerDue || 0) - paid,
              0,
            ),
          };
        })
      ...
      if (requested > grandRemaining + 0.001) {
        throw new Error(
          `الدفعة أكبر من الرصيد المتبقي (${grandRemaining.toFixed(2)} CAD)`,
        );
      }
      ...
      if (oldBalanceRemaining > 0 && left > 0.0001) {
        ...
        customer.oldBalancePaid = +(
          oldBalancePaid + oldBalanceAllocation
        ).toFixed(2);
...
app.post("/api/transactions/:id/payments", auth, (req, res) => {
  ...
  const payment = mutate((s) => {
    ...
    const already = s.payments
      .filter((p) => p.transactionId === t.id)
      .reduce((a, p) => a + Number(p.amount), 0);
    if (already + n > Number(t.totalCustomerDue) + 0.001)
    ...
    s.payments.push(p);
    audit(s, req.user.id, "PAYMENT", "TRANSACTION", t.id, { amount: n });
    return p;
  });
...
app.patch("/api/transactions/:id", auth, (req, res) => {
  try {
    const updated = mutate((s) => {
      const transaction = s.transactions.find(
        (item) => item.id === req.params.id && !item.isDeleted,
      );
      ...
      if (
        ![amount, cost, finalRate, fee].every(Number.isFinite) ||
        amount <= 0 ||
        cost <= 0 ||
        finalRate <= 0 ||
        fee < 0
      ...
      transaction.beneficiaryReceives = +(
        transaction.feeMethod === "DEDUCT" ? Math.max(amount - fee, 0) : amount
      ).toFixed(2);
      transaction.exchangeProfit = +exchangeProfit.toFixed(2);
      transaction.totalProfit = +(exchangeProfit + fee).toFixed(2);
      transaction.totalCustomerDue = +(
        transaction.feeMethod === "ADD"
          ? baseCustomerDue + fee
          : baseCustomerDue
      ).toFixed(2);
      ...
      audit(s, req.user.id, "UPDATE", "TRANSACTION", transaction.id, {
        oldData,
        newData: { ...transaction },
      });
      return transaction;
    ...
    if (!updated)
      return res.status(404).json({ message: "الحوالة غير موجودة" });
    res.json(updated);
  } catch (error) {
...
app.delete("/api/transactions/:id", auth, (req, res) => {
  const deleted = mutate((s) => {
    ...
    audit(s, req.user.id, "DELETE", "TRANSACTION", transaction.id, {
      softDelete: true,
    });
    return transaction;
  });
...
app.patch("/api/payments/:id", auth, (req, res) => {
  try {
    const updated = mutate((s) => {
      ...
      const transaction = s.transactions.find(
        (item) => item.id === payment.transactionId && !item.isDeleted,
      );
      ...
      const totalPaid = s.payments
        .filter(
          (item) => item.transactionId === transaction.id && !item.isDeleted,
        )
      ...
      audit(s, req.user.id, "UPDATE", "PAYMENT", payment.id, {
        oldData,
        newData: { ...payment },
      });
      return payment;
...
app.delete("/api/payments/:id", auth, (req, res) => {
  const deleted = mutate((s) => {
    ...
    payment.deletedAt = now();
    payment.deletedBy = req.user.id;
    audit(s, req.user.id, "DELETE", "PAYMENT", payment.id, {
      softDelete: true,
    });
    return payment;
  });
...
async function fetchOfficialRate(baseCurrency, quoteCurrency) {
  ...
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent": "AlAboud-Cloud/15.3.63",
    },
  });
  if (!response.ok)
...
async function fetchSyrianPoundRate() {
  ...
  if (data.result !== "success")
    throw new Error(data["error-type"] || "SYP provider failed");
  const rate = Number(data.rates?.SYP);
  if (!Number.isFinite(rate) || rate <= 0)
    throw new Error("SYP rate is unavailable");
  return {
    rate,
...
async function fetchGoldPriceCad() {
  ...
  if (!response.ok)
    throw new Error(`Gold provider returned ${response.status}`);
  const data = await response.json();
...
app.get("/api/profits", auth, (req, res) => {
  ...
  const transactions = s.transactions.filter(
    (t) => t.status !== "CANCELLED" && inRange(t.createdAt),
  );
  const expenses = s.expenses.filter((e) => inRange(e.date || e.createdAt));

  const exchangeProfit = transactions.reduce(
    (a, t) => a + Number(t.exchangeProfit || 0),
    0,
  );
  const transferFees = transactions.reduce(
    (a, t) => a + Number(t.transferFee || 0),
    0,
  );
  const grossProfit = transactions.reduce(
    (a, t) => a + Number(t.totalProfit || 0),
    0,
  );
...
app.post("/api/exchange-rates/refresh", auth, async (req, res) => {
  try {
    ...
    res
      .status(502)
      .json({ message: "تعذر تحديث أسعار الصرف", error: error.message });
  }
});
...
app.get("/api/exchange-rates", auth, (_req, res) => {
  ...
  for (const rate of s.exchangeRates
    .slice()
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))) {
...
app.post("/api/exchange-rates", auth, (req, res) => {
  ...
  const rate = mutate((s) => {
    ...
    audit(s, req.user.id, "CREATE", "EXCHANGE_RATE", x.id, {
...
app.get("/api/general-debts", auth, (req, res) => {
  const store = readStore();
  const debts = Array.isArray(store.generalDebts) ? store.generalDebts : [];
  const payments = Array.isArray(store.generalDebtPayments)
    ? store.generalDebtPayments
    : [];
  ...
  const totals = {
    receivable: rows
      ...
      .reduce((s, x) => s + safeNumber(x.remaining), 0),
    payable: rows
...
app.get("/api/general-debts/:id/payments", auth, (req, res) => {
  const store = readStore();
  const list = (
    Array.isArray(store.generalDebtPayments) ? store.generalDebtPayments : []
  )
...
app.post("/api/general-debts/:id/payments", auth, (req, res) => {
  ...
  const store = readStore();
  const debt = (
    Array.isArray(store.generalDebts) ? store.generalDebts : []
  ).find((item) => item.id === req.params.id);
  ...
  try {
    ...
    const transactions = (
      ...
      .map((transaction) => {
        ...
        const costCad =
          transaction.direction === "USD_TO_CAD"
            ? usdAmount * costRate
            : usdAmount * costRate;
      ...
      .sort((a, b) =>
        String(a.transferDate).localeCompare(String(b.transferDate)),
      );
    ...
    res.json({
      company: (() => {
        const company = (
          Array.isArray(store.companies) ? store.companies : []
        ).find((item) => item.id === req.user.companyId);
        return {
          name: company?.name || "شركة العبود للتجارة",
          nameEn: "",
          logoDataUrl: company?.logoDataUrl || "",
        };
      })(),
...
app.get("/api/transactions/:id/invoice", auth, (req, res) => {
  try {
    ...
    const customer = (Array.isArray(store.customers)
      ? store.customers
      : []
...
app.get("/api/partners", auth, (_req, res) => {
  ...
  const transactions = Array.isArray(store.partnerTransactions)
    ? store.partnerTransactions
    : [];
  const payments = Array.isArray(store.partnerPayments)
    ? store.partnerPayments
    : [];
...
app.post("/api/partners", auth, (req, res) => {
  ...
  if (!name)
    return res.status(400).json({ message: "اسم المورد أو الشركة مطلوب" });
  ...
  const partner = mutate((store) => {
    ...
    audit(store, req.user.id, "CREATE", "PARTNER", item.id, {
      name: item.name,
    });
    return item;
  });
...
app.get("/api/partners/:id", auth, (req, res) => {
  const store = readStore();
  const partner = (Array.isArray(store.partners) ? store.partners : []).find(
    (item) => item?.id === req.params.id,
  );

  if (!partner)
    return res.status(404).json({ message: "المورد أو الشركة غير موجود" });

  const transactions = (
    Array.isArray(store.partnerTransactions) ? store.partnerTransactions : []
  )
  ...
  const payments = (
    Array.isArray(store.partnerPayments) ? store.partnerPayments : []
  )
    .filter((item) => item.partnerId === partner.id)
    .sort((a, b) =>
      String(b.date || b.createdAt).localeCompare(
  ...
  res.json({
    ...
    totals: {
      ...
      net: +(
        Math.max(receivable - received, 0) - Math.max(payable - paid, 0)
      ).toFixed(2),
    },
  });
});
...
app.get("/api/partners/:id/statement", auth, (req, res) => {
  ...
  const transactions = (
    ...
    (item) =>
      item.partnerId === partner.id && inRange(item.date || item.createdAt),
  );
  const payments = (
    Array.isArray(store.partnerPayments) ? store.partnerPayments : []
  ).filter(
...
app.get("/api/expenses", auth, (_req, res) =>
  res.json(readStore().expenses.slice().reverse()),
);
app.post("/api/expenses", auth, (req, res) => {
  ...
  const n = Number(amount);
  if (!title || !Number.isFinite(n) || n <= 0)
  ...
  const e = mutate((s) => {
    ...
    s.expenses.push(x);
    audit(s, req.user.id, "CREATE", "EXPENSE", x.id);
    return x;
  });
  res.status(201).json(e);
});
app.get("/api/capital", auth, (_req, res) =>
  res.json(readStore().capitalMovements.slice().reverse()),
);
app.post("/api/capital", auth, (req, res) => {
  ...
  if (!["IN", "OUT"].includes(type) || !Number.isFinite(n) || n <= 0)
    return res.status(400).json({ message: "Invalid capital movement" });
  const m = mutate((s) => {
    const x = {
      id: id(),
      type,
    ...
    s.capitalMovements.push(x);
    audit(s, req.user.id, "CREATE", "CAPITAL", x.id);
    return x;
  });
...
app.patch("/api/capital/:id", auth, (req, res) => {
  ...
  const updated = mutate((store) => {
    const item = store.capitalMovements.find(
    ...
    audit(store, req.user.id, "UPDATE", "CAPITAL", item.id, {
      type: item.type,
      amount: item.amount,
    });
    return item;
  ...
  if (!updated)
    return res.status(404).json({ message: "حركة رأس المال غير موجودة" });
  res.json(updated);
});

app.delete("/api/capital/:id", auth, (req, res) => {
  const removed = mutate((store) => {
    const index = store.capitalMovements.findIndex(
      (entry) => entry.id === req.params.id,
    );
    if (index < 0) return null;
    const [item] = store.capitalMovements.splice(index, 1);
    audit(store, req.user.id, "DELETE", "CAPITAL", item.id, {
      type: item.type,
      amount: item.amount,
    });
    return item;
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`AlAboud Enterprise Cloud v15.3.63 running on port ${PORT}`);
    ...
    const runHourlyRateRefresh = async () => {
      try {
        ...
        console.log(
          `Hourly exchange-rate refresh: ${successCount}/${results.length} updated`,
        );
      } catch (error) {
