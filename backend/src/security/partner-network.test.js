...
(async()=>{
  ...
  await assert.rejects(()=>assertSafePartnerUrl("http://localhost/admin"),/PARTNER_PRIVATE_HOST/);
  await assert.rejects(()=>assertSafePartnerUrl("http://169.254.169.254/latest/meta-data"),/PARTNER_PRIVATE_IP/);
  ...
  await assert.rejects(()=>pinnedPartnerFetch("http://127.0.0.1/"),/PARTNER_PRIVATE_IP/);
