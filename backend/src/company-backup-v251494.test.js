...
const root = {
  ...
  partners:[], partnerTransactions:[], partnerPayments:[], partnerSyncLogs:[], notificationActions:[], auditLogs:[], monthlyInventories:[],
  treasuryMovements:[{id:"tm1",companyId:"c1",branchId:"b1",sourceKey:"MANUAL:c1",direction:"IN",currency:"CAD",quantity:250,createdAt:"2026-08-27T00:00:00.000Z"},{id:"tm9",companyId:"c2",branchId:"b9",sourceKey:"MANUAL:c2",direction:"IN",currency:"CAD",quantity:999,createdAt:"2026-08-27T00:00:00.000Z"}], integrationLogs:[],
...
assert.equal(data.treasuryMovements.length,1,"company treasury movements must be exported");
assert.equal(data.treasuryMovements[0].id,"tm1");
...
assert.equal(restored.treasuryMovements.filter(row=>row.companyId==="c1").length,1,"company treasury movements must be restored");
assert.equal(restored.treasuryMovements.find(row=>row.id==="tm9").companyId,"c2","other-company treasury must remain untouched");
...
for(const key of ["customers","transactions","payments","expenses","capitalMovements","exchangeRates","generalDebts","generalDebtPayments","partners","partnerTransactions","partnerPayments","partnerSyncLogs","notificationActions","auditLogs","monthlyInventories","treasuryMovements"]){
