...
import api from "./api";
export default function LoginShell({onLogin,forcePasswordChange=false}){
  ...
  const [forcedPassword,setForcedPassword]=useState({required:Boolean(forcePasswordChange),currentPassword:"",newPassword:"",confirmPassword:""});
  async function saveSession(data){
    ...
    if(data.user?.mustChangePassword){
      setForcedPassword(current=>({...current,required:true,currentPassword:current.currentPassword||password}));
      return;
    }
    ...
    try{
      ...
      if(native?.requestBiometricLogin){
        ...
        if(native.isBiometricEnabled?.()) native.saveBiometricToken?.(response.data.token,userJson,response.data.deviceId);
        else native.enableBiometricLogin?.(response.data.token,userJson,response.data.deviceId);
      }
    }catch{}
    onLogin();
  }
  ...
  async function submitForcedPasswordChange(event){
    ...
    if(forcedPassword.newPassword!==forcedPassword.confirmPassword){setError("تأكيد كلمة المرور غير مطابق");return;}
    setBusy(true);
    try{
      const {data}=await api.post("/auth/change-password",{currentPassword:forcedPassword.currentPassword,newPassword:forcedPassword.newPassword});
      const user={...JSON.parse(localStorage.getItem("afs_user")||"{}"),mustChangePassword:false};
      localStorage.setItem("afs_user",JSON.stringify(user));
      setForcedPassword({required:false,currentPassword:"",newPassword:"",confirmPassword:""});
      if(data?.mustChangePassword===false)onLogin();
    }catch(error){setError(error.response?.data?.message||"تعذر تغيير كلمة المرور المؤقتة")}finally{setBusy(false)}
  }
  useEffect(()=>{api.get("/auth/registration-status",{suppressToast:true}).then(({data})=>{const enabled=Boolean(data?.enabled);setRegistrationEnabled(enabled);if(!enabled)setMode(current=>current==="register"?"login":current)}).catch(()=>setRegistrationEnabled(false))},[]);
  useEffect(()=>{const handler=async event=>{if(!event.detail?.ok){setError(event.detail?.message||"تعذر الدخول بالبصمة أو الوجه");return}try{setBusy(true);const {data}=await api.get("/auth/session",{suppressToast:true});await saveSession(data)}catch(error){setError(error.response?.data?.message||"تعذر الدخول بالبصمة أو الوجه")}finally{setBusy(false)}};window.addEventListener("alaboud-biometric-login-result",handler);return()=>window.removeEventListener("alaboud-biometric-login-result",handler)},[]);
  useEffect(()=>{
  ...
  if(forcedPassword.required)return <div className="login"><form className="panel public-account-panel" onSubmit={submitForcedPasswordChange}><img className="login-company-logo" src="/alaboud-company-logo.webp" alt="شركة العبود التجارية"/><h1>تغيير كلمة المرور المؤقتة</h1><p>لحماية الحساب، يجب اختيار كلمة مرور جديدة قبل متابعة استخدام النظام.</p><input type="password" value={forcedPassword.currentPassword} onChange={e=>setForcedPassword({...forcedPassword,currentPassword:e.target.value})} placeholder="كلمة المرور الحالية" required/><input type="password" value={forcedPassword.newPassword} onChange={e=>setForcedPassword({...forcedPassword,newPassword:e.target.value})} placeholder="كلمة المرور الجديدة — 12 حرفًا قوية" required/><input type="password" value={forcedPassword.confirmPassword} onChange={e=>setForcedPassword({...forcedPassword,confirmPassword:e.target.value})} placeholder="تأكيد كلمة المرور الجديدة" required/>{error&&<div className="error">{error}</div>}<button disabled={busy}>{busy?"جاري الحفظ...":"تغيير كلمة المرور والدخول"}</button></form></div>;
