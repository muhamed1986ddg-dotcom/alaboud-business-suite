...
function Login({onLogin}){
  ...
  const initialResetToken=initialParams.get("resetToken")||"";
  const initialResetEmail=initialParams.get("email")||"";
  ...
  const biometricAvailable=Boolean(window.AlAboudNative?.isBiometricAvailable?.());
  const biometricEnabled=Boolean(window.AlAboudNative?.hasBiometricToken?.());
  async function saveSession(data){
    localStorage.setItem("token",data.token);localStorage.setItem("user",JSON.stringify(data.user));
    try{if(window.AlAboudNative?.isBiometricEnabled?.()){const response=await api.post("/auth/biometric-token");window.AlAboudNative.saveBiometricToken(response.data.token,JSON.stringify(data.user));}}catch{}
    onLogin();
  }
  async function submitLogin(e){
    e.preventDefault();setError("");setMessage("");if(!accepted){setError("يجب الموافقة على سياسة الخصوصية وشروط الاستخدام");return}
  ...
  async function submitForgot(e){
    ...
    try{const {data}=await api.post("/auth/forgot-password",{email});setMessage(data.message||"تم إرسال تعليمات الاستعادة");if(data.devResetToken)setResetToken(data.devResetToken)}
    catch(error){setError(error.response?.data?.message||"تعذر إرسال طلب الاستعادة")}finally{setBusy(false)}
  }
  async function submitReset(e){
    e.preventDefault();setError("");setMessage("");
    if(newPassword!==confirmNewPassword){setError("تأكيد كلمة المرور غير مطابق");return}
    setBusy(true);
    try{const {data}=await api.post("/auth/reset-password",{email,token:resetToken,newPassword});setMessage(data.message||"تم تغيير كلمة المرور");setPassword("");setNewPassword("");setConfirmNewPassword("");window.history.replaceState({},"",window.location.pathname);setTimeout(()=>setMode("login"),700)}
    catch(error){setError(error.response?.data?.message||"تعذر إعادة تعيين كلمة المرور")}finally{setBusy(false)}
  }
  ...
  if(mode==="forgot")return <div className="login"><form className="panel public-account-panel" onSubmit={submitForgot}><img className="login-company-logo" src="/alaboud-company-logo.webp" alt="شركة العبود التجارية"/><h1>نسيت كلمة المرور</h1><p>أدخل بريدك الإلكتروني وسنرسل لك رابطًا صالحًا لمدة 15 دقيقة.</p><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="البريد الإلكتروني" required/>{message&&<div className="success">{message}</div>}{error&&<div className="error">{error}</div>}<button disabled={busy}>{busy?"جاري الإرسال...":"إرسال رابط الاستعادة"}</button>{resetToken&&<button type="button" className="account-mode-button" onClick={()=>setMode("reset")}>إدخال كلمة مرور جديدة</button>}<button type="button" className="account-mode-button" onClick={()=>{setMode("login");setError("");setMessage("")}}>العودة إلى تسجيل الدخول</button></form></div>;
  if(mode==="reset")return <div className="login"><form className="panel public-account-panel" onSubmit={submitReset}><img className="login-company-logo" src="/alaboud-company-logo.webp" alt="شركة العبود التجارية"/><h1>تعيين كلمة مرور جديدة</h1><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="البريد الإلكتروني" required/><input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="كلمة المرور الجديدة — 12 حرفًا قوية" required/><input type="password" value={confirmNewPassword} onChange={e=>setConfirmNewPassword(e.target.value)} placeholder="تأكيد كلمة المرور الجديدة" required/>{message&&<div className="success">{message}</div>}{error&&<div className="error">{error}</div>}<button disabled={busy}>{busy?"جاري الحفظ...":"حفظ كلمة المرور الجديدة"}</button><button type="button" className="account-mode-button" onClick={()=>setMode("login")}>العودة</button></form></div>;
  return <div className="login"><form className="panel public-account-panel" onSubmit={mode==="login"?submitLogin:submitRegister}><img className="login-company-logo" src="/alaboud-company-logo.webp" alt="شركة العبود التجارية"/><h1>{mode==="login"?"تسجيل الدخول":"إنشاء حساب شركة جديد"}</h1><p className="login-company-en">ALABOUD BUSINESS SUITE</p>{mode==="login"?<><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="البريد الإلكتروني" required/><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="كلمة المرور" required/><button type="button" className="account-mode-button" onClick={()=>{setMode("forgot");setError("");setMessage("")}}>نسيت كلمة المرور؟</button></>:<><input value={form.ownerName} onChange={e=>setForm({...form,ownerName:e.target.value})} placeholder="اسم صاحب الحساب" required/><input value={form.companyName} onChange={e=>setForm({...form,companyName:e.target.value})} placeholder="اسم الشركة" required/><input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="البريد الإلكتروني" required/><input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="رقم الهاتف"/><input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} placeholder="كلمة المرور — 12 حرفًا قوية" required/><input type="password" value={form.confirmPassword} onChange={e=>setForm({...form,confirmPassword:e.target.value})} placeholder="تأكيد كلمة المرور" required/></>}<label className="legal-consent"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/><span>أوافق على سياسة الخصوصية وشروط الاستخدام</span></label>{message&&<div className="success">{message}</div>}{error&&<div className="error">{error}</div>}<button disabled={busy}>{busy?"جاري التنفيذ...":mode==="login"?"تسجيل الدخول":"إنشاء الحساب والدخول"}</button>{mode==="login"&&biometricEnabled&&<button className="biometric-login-button" type="button" onClick={()=>window.AlAboudNative.requestBiometricLogin()}>👆 الدخول بالبصمة أو الوجه</button>}<button className="account-mode-button" type="button" onClick={()=>{setMode(mode==="login"?"register":"login");setError("");setMessage("")}}>{mode==="login"?"مستخدم جديد؟ إنشاء حساب شركة":"لدي حساب بالفعل — تسجيل الدخول"}</button></form></div>
}
