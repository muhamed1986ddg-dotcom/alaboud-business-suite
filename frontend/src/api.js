...
api.interceptors.request.use(config=>{
  ...
  config.headers["X-Alaboud-Client-Version"]="25.4.1";
  ...
  config.timeout=method==="get"?30000:15000;
...
function errorToastMessage(method,error){
  if(error?.code==="ECONNABORTED"||/timeout/i.test(String(error?.message||""))){
    return "لم يصل تأكيد العملية خلال 15 ثانية. تحقق من الاتصال وحالة العملية قبل إعادة المحاولة.";
  }
